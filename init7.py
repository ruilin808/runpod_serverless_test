...
MS-Swift Compatible Implementation
"""
...
# Import TEDS - add this to your requirements
try:
    from table_recognition_metric import TEDS
    TEDS_AVAILABLE = True
except ImportError:
    print("Warning: TEDS not available. Install with: pip install table-recognition-metric")
    TEDS_AVAILABLE = False


class TEDSEvaluationCallback(TrainerCallback):
    """
    MS-Swift compatible TEDS evaluation callback
    Runs TEDS evaluation every eval_steps during training
    """
    
    def __init__(self, validation_dataset, template, max_samples=50):
        """
        Args:
            validation_dataset: Dataset with 'images' and 'html_table' fields
            template: Swift template for inference
            max_samples: Maximum number of samples to evaluate (for speed)
        """
        self.validation_dataset = validation_dataset
        self.template = template
        self.max_samples = max_samples
        self.teds = TEDS() if TEDS_AVAILABLE else None
        self.logger = get_logger()
        
    def on_evaluate(self, args: TrainingArguments, state: TrainerState, 
                   control: TrainerControl, model=None, **kwargs):
        """Called during evaluation - runs TEDS evaluation"""
        
        if not TEDS_AVAILABLE or self.teds is None:
            return
            
        try:
            # Run TEDS evaluation
            teds_score = self._evaluate_teds(model, state.global_step)
            
            # Log to tensorboard if available
            if hasattr(kwargs.get('logs', {}), '__setitem__'):
                kwargs['logs']['eval_teds'] = teds_score
                
            self.logger.info(f"Step {state.global_step}: TEDS Score = {teds_score:.4f}")
            
        except Exception as e:
            self.logger.error(f"TEDS evaluation failed: {e}")
    
    def _evaluate_teds(self, model, step: int) -> float:
        """Run TEDS evaluation on a subset of validation data"""
        
        if not hasattr(self.validation_dataset, '__len__'):
            return 0.0
            
        # Limit samples for speed
        num_samples = min(self.max_samples, len(self.validation_dataset))
        indices = torch.randperm(len(self.validation_dataset))[:num_samples].tolist()
        ...
        with torch.no_grad():
            for idx in indices:
                try:
                    ...
                    # Generate prediction
                    pred_html = self._generate_html(model, sample)
                    ...
                    self.logger.warning(f"Sample {idx} failed: {e}")
                    continue
        ...
        return sum(teds_scores) / len(teds_scores) if teds_scores else 0.0
    
    def _generate_html(self, model, sample) -> str:
        """Generate HTML prediction for a single sample"""
        
        # Prepare input in Swift format
        messages = [
            {
                'role': 'user',
        ...
        encoded = self.template.encode(infer_input)
        ...
        if 'pixel_values' in encoded:
            pixel_values = encoded['pixel_values'].unsqueeze(0).to(model.device)
        else:
            pixel_values = None
        ...
        response_text = self.template.tokenizer.decode(
            response_ids, 
...
def main():
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        save_steps=100,
        eval_strategy='steps',
        eval_steps=2,  # TEDS will run every 100 steps
        gradient_accumulation_steps=gradient_accumulation_steps,
        num_train_epochs=3,
        metric_for_best_model='eval_teds',  # Use TEDS as primary metric
        greater_is_better=True,  # Higher TEDS is better
        save_total_limit=3,
        logging_steps=10,
    ...
    # Setup TEDS callback
    callbacks = []
    if TEDS_AVAILABLE and len(val_processed) > 0:
        teds_callback = TEDSEvaluationCallback(
            validation_dataset=val_processed,
            template=template,
            max_samples=min(50, len(val_processed))  # Limit for speed
        )
        callbacks.append(teds_callback)
        logger.info("TEDS evaluation callback added")
    ...
    trainer = Seq2SeqTrainer(
        ...
        template=template,
        callbacks=callbacks,  # Add TEDS callback
    )
    ...
    plot_images(visual_loss_dir, training_args.logging_dir, ['train/loss', 'eval_teds'], 0.9)
